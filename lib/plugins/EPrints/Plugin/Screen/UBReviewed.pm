...
EPrints::Plugin::Screen::UBReviewed

=cut


package EPrints::Plugin::Screen::UBReviewed;
...
sub properties_from
{
	...
	$processor->{dataset} = $repo->dataset( "reviewed" );
...
sub get_filters
{
	...
	return(
		{ meta_fields => [qw( eprint_status )], value => "reviewed" },
	);
}
...
sub render_top_bar
{
	...
	if( $user->is_set( "editperms" ) )
	{
		...
		$div->appendChild( $repo->html_phrase( 
			"cgi/users/reviewed:reviewed_scope",
	...
	my %options = (
		...
		title => $repo->html_phrase( "Plugin/Screen/UBReviewed:help_title" ),
		content => $repo->html_phrase( "Plugin/Screen/UBReviewed:help" ),
		collapsed => 1,
